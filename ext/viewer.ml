...
let apply_scale content host factor =
  set_prop content "transform" (Printf.sprintf "scale(%.4f)" factor);
  raf (fun _ ->
    let h = Jv.to_float (Jv.get
      (Jv.call (El.to_jv content) "getBoundingClientRect" [||]) "height") in
    set_prop host "min-height" (Printf.sprintf "%.0fpx" h))

let zoom_to content host ~prev ~next origin_y =
  let b = El.to_jv (body ()) in
  let top = Jv.to_float (Jv.get b "scrollTop") in
  apply_scale content host next;
...
let rec setup_viewer (epub : Epub.t) z =
  ...
  apply_scale content content_host 1.0;
  ...
  let update_zoom () =
    ...
    apply_scale content content_host !zoom;
  ...
  let do_zoom ?origin f =
    ...
    let prev = !zoom in zoom := clamp f;
    zoom_to content content_host ~prev ~next:!zoom (match origin with Some y -> y | None -> mid);
    update_zoom () in
